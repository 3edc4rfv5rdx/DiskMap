...
# file app/build.gradle.kts reads it from.
#
...
DNAME="CN=$PROJECT, OU=$PROJECT, O=$PROJECT, C=UA"
...
command -v keytool >/dev/null 2>&1 || die "keytool not found — it comes with the JDK."
...
echo
[ "$STORE_PASS" = "$STORE_PASS2" ] || die "They differ."
[ ${#STORE_PASS} -ge 6 ] || die "keytool wants at least 6 characters."
